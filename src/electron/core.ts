...
# any app launch will use a stale or missing mcp.json. Known limitation.
cd "$WORK_DIR" && CLAUDE_CONFIG_DIR='${configDir}' claude --mcp-config '${configDir}/mcp.json' --strict-mcp-config
`;
...
export async function launchProfile(profile: Profile, directory?: string): Promise<void> {
  ...
  const script = [
    ...
    `    write text "cd '${workDir}' && CLAUDE_CONFIG_DIR='${configDir}' claude --mcp-config '${mcpConfigPath}' --strict-mcp-config"`,
    "  end tell",
    "end tell",
  ].join("\n");
